...
/// A single material color swatch (a family + shade combination).
struct MaterialSwatch: Identifiable, Hashable {
    let id = UUID()
    let familyName: String
    let shadeName: String
    let hex: String

    var displayName: String { "\(familyName) \(shadeName)" }

    var color: Color { Color(hex: hex) }

    /// White text reads better on dark swatches, black on light ones.
    var preferredTextColor: Color {
        let rgb = RGB(hex: hex) ?? RGB(r: 0, g: 0, b: 0)
        // Relative luminance (sRGB, perceptual).
        let lum = 0.2126 * rgb.rLinear + 0.7152 * rgb.gLinear + 0.0722 * rgb.bLinear
        return lum > 0.5 ? .black : .white
    }
}

/// A named family of shades (e.g. "Red" 50...900 + accents).
struct MaterialFamily: Identifiable, Hashable {
    let id = UUID()
    let name: String
    let swatches: [MaterialSwatch]

    /// The 500 shade (or middle shade) used to represent the family.
    var representative: MaterialSwatch {
        swatches.first(where: { $0.shadeName == "500" }) ?? swatches[swatches.count / 2]
    }
}

/// The full Material Design 2014 color palette.
enum MaterialPalette {

    static let families: [MaterialFamily] = raw.map { entry in
        MaterialFamily(
            name: entry.name,
            swatches: entry.shades.map { shade in
                MaterialSwatch(familyName: entry.name, shadeName: shade.0, hex: shade.1)
            }
        )
    }

    /// Flat list of every swatch across every family.
    static let allSwatches: [MaterialSwatch] = families.flatMap { $0.swatches }

    private typealias Shade = (String, String)
    private struct Entry { let name: String; let shades: [Shade] }

    private static let raw: [Entry] = [
        Entry(name: "Red", shades: [
